...
function NavAside(props) {
  ...
  const navList = navObj.map(item => {
      const itemClassName = item.name === choosenContent ? activeClassName : defaultClassName;

      return <Item key={item.key} item={item} itemClassName={itemClassName} switchContent={switchContent} />;
  });
